...
- **Self-search is now built and verified live, not just specified — Section 6.3 and Section 1.5 rewritten accordingly.** All three cases (zero-match/create, one match, several matches via the disambiguation list) confirmed against a real preview deployment with real Airtable reads and writes, not a typecheck. The self-correcting duplicate-prevention property is empirically confirmed, not just designed. Code sits in an unmerged PR — proven working, not yet on `main`.
- **A real, previously-unknown bug in `Quadrant Tag` was found and fixed during this same verification** (Section 7) — the formula's "pending" check only matched the literal `"Not Yet Checked"` value, not a genuinely blank field, so every brand-new, never-checked record was misclassified as the worst case (`Flagged-Flagged`) instead of pending. This wasn't self-search-specific — it almost certainly affected cold-outreach leads pre-check too. Fixed and confirmed live on the exact record that exposed it.
- **Abuse protection on the self-search endpoint moved from flagged to built and verified (Section 6.3), removed from Section 11's gap list.** A new dedicated `Requester IP` field, exact-match rate limiting reusing existing `Compliance Lookup Events` infrastructure rather than a new service, and both known limitations documented in code rather than hidden. Confirmed live with unusual precision: the limit fired on exactly the 11th request after exactly 10 succeeded, verified by reading the actual event count directly rather than trusting the observed behavior alone.
- **The Preview PAT question is resolved, not just surfaced — and not the way either original option would have.** Rather than leaving Production's real token permanently write-scoped, or reverting it every time and repeating the friction, Preview now has its own separate, dedicated token with write access, while the original token — the one Production actually uses — is back to read-only, confirmed directly in Airtable's own settings. Both verified working correctly against real data afterward, not just configured and assumed.
- **Vercel's preview-collaboration toolbar is documented and disabled for this project (Section 9.7)** — it repeatedly intercepted testing input during self-search verification, disguised as the app itself misbehaving. The setting (labeled "Pre-Production Deployments" in the current UI) was inheriting a team-level default, not explicitly on; now explicitly overridden to Off at the project level, with zero effect on Production. Scoped to this project specifically — a future, separate Vercel project would need the same override applied to it. A related testing practice (direct API calls for exact-count verification, UI clicks reserved for confirming real user experience) is recorded alongside it.
- **A critical pre-merge gap was found before it could become a real regression, not after (Section 9.7).** Merging PR #2 as-is would have deployed self-search's write-dependent code to Production while Production's own token was still read-only — a direct, unreconsidered consequence of the earlier credential-separation work, which correctly narrowed Production's scope for an unverified feature but was never revisited against the moment that feature actually merges. Production's token needs matching write scope added before merging, not after — a durable pre-merge check is now recorded for every future write-dependent feature, not just this one.
- **Vercel's Firewall bot-protection settings are documented, not just set.** Bot Protection and AI Bots are both on Log, deliberately, because Sendr's expected fetch pattern (Section 2) looks exactly like what the stricter settings would block, and Sendr isn't connected yet to verify that. Revisit once it is.
...
1. **GHL → Airtable** (lead intake / sequence sync). Never built — confirmed directly against Airtable's live automations list, which shows only two automations total (`Trigger Sendr Generation`, `Flag Stale Records`), neither GHL-related. If the front end assumes `Sequence Status` is fresh, that assumption is currently false.
...
**The webhook payload spec is corrected here, not just flagged as stale anymore.** The original spec named `Company Name`, `Phone (Match Key)`, `WSIB Status`, `Corporate Status` — `Company Name` hasn't existed since the trade-name/legal-name split (Section 10). Corrected field list: `Trade/Operating Name`, `Legal Name`, `Phone (Match Key)`, `WSIB Status`, `Corporate Status`. This is a minimal, direct fix — swapping the one wrong field name for its real replacements — not a full redesign of what Sendr needs. **The exact request format Sendr actually expects (auth style, JSON shape) still can't be finalized without either a live account or Sendr's own published documentation** — that piece stays genuinely open, tracked in Section 11.
...
| `Stated WSIB Expiry Date` (`fldOP2iQoJBcAsJDw`) | Real, manually-entered date — replaced a flawed flat formula (Section 10). | **Reads for display and for logic** — the actual date shown, and the basis for the freshness/urgency framing in Section 8.2, independent of `Quadrant Tag` (Section 7 — resolved, not gating this). |
...
| `Jurisdiction` (`fldv5Rh1GakCUOYzX`) | Forward-compatible scaffolding (Section 1.4), Ontario-only for now. | **Reads for display**, optional/low-priority — could reasonably be omitted from a first version entirely. Note: among the 9 sparse test records, one has a stray "Quebec" value rather than blank or "Ontario" — almost certainly someone testing the field, not a real second jurisdiction being served. Worth a one-time backfill before this matters, per Section 11. |
...
### 6.3 Self-search — built and verified live, not just specified

Per Section 1.1, this is a deliberate, intended feature: a visitor searching for their own business, even unsuccessfully, is a lead-capture signal worth acting on. **This is now built, deployed to a real preview environment, and verified against live data across all three real cases — not a spec awaiting implementation.** It's sitting in an unmerged PR, not yet on `main` — see the note at the end of this section for exactly what that means and doesn't mean.

**Page architecture — confirmed and built as designed.** Self-search lives on a standalone home page, with the search itself in that page's own nav — not embedded in the nav of the personalized `/r/:token` results page. The personalized page carries a small, deliberate text link ("Looking to check another business? Search here") rather than a persistent search box, so a lead's own results page doesn't read as a standing invitation to go check other businesses. A search result — a single match, or a selection made from a disambiguation list — routes into the same shared `/r/:token` page every other path already uses; confirmed live, no separate results display was built. This mattered beyond tidiness: a second, independent implementation of the consent-gating logic (Section 8.3) would have risked quietly drifting out of sync with the one already built and verified — the exact failure pattern `Quadrant Tag` and the repo's own tier logic fell into independently, before anyone noticed either had happened.

**`Compliance Lookup Events` is now genuinely connected, confirmed by direct inspection of real logged entries** — not just "built to be connected." Every self-search attempt logs here with the real `Event Type = Search` value, correctly linked back to the matched or newly-created `Compliance Records` entry.

**`SearchModal.tsx` has been repurposed, not just flagged for repurposing.** It now calls the real endpoint and renders both the search form and the disambiguation list against live data — confirmed visually, not just by a passing typecheck. It no longer touches the old mock array.

**The matching strategy — confirmed working exactly as designed, including the specific case that mattered most.** Normalized comparison (lowercase, trimmed, punctuation stripped) against `Trade/Operating Name`, using either an exact match or a bidirectional prefix match — the input is a prefix of the stored name, or the stored name is a prefix of the input — always scoped to the jurisdiction selected in the search. Deliberately not fuzzy or similarity-scored. **Verified live:** a search for "Bright Electrical" against two real records — "Bright Electrical" and "Bright Electrical Group Inc." — correctly returned both as a disambiguation list, confirming the prefix rule fires correctly, not just the simpler exact-match path. The reasoning behind the strictness, unchanged: the risk this guards against isn't a consent bypass — the gate in Section 8.3 holds regardless of match quality — it's a *wrong-business* risk, and a tight match that sometimes returns nothing is safe, since "no match yet" is already an acceptable outcome (Section 1.1).

**`Entry Channel` (`fldHyu4DoQwsSSSuS`) is confirmed live, not just created.** A real self-search test created a new record and it correctly carried `Self-Search` (`selWyyaGdkV1vc3uW`) — verified by reading the record directly, not assumed from the code. The existing cold-outreach scraping still needs updating to set `Cold Outreach` at creation the same way; that side hasn't been touched by this work.

**The write path is built and verified, including the specific behavior that mattered most.** The new endpoint follows the same server-side pattern as `api/records/[token].ts` — no writable credential ever reaches the client. Confirmed live: a zero-match search created a real new `Compliance Records` entry with the correct `Lookup Status = Not Started`, the correct searched name and jurisdiction, and the correct `Entry Channel`. **The self-correcting duplicate-prevention property is now empirically confirmed, not just a designed intention** — the same search term run again later matched the record the first search had already created, rather than creating a second one, visible directly in the `Compliance Lookup Events` log (first attempt: "0 matches — new record created"; later attempts: "1 match").

**The PAT write-scope requirement is resolved, not open — and resolved better than the original binary question posed.** The question was whether `AIRTABLE_PAT` should stay permanently write-scoped on Preview, or be reverted after testing. Neither — Preview now has its own separate, dedicated token (`TradeGRC Preview Write-Test`) with read and write access, isolated from Production entirely. The original token (`TradeGRC Vercel Proxy`), the one Production's live code actually uses, is back to read-only — confirmed directly in Airtable's token settings, not assumed. This required no code changes anywhere: Vercel supports different values for the same environment variable name per environment, so `AIRTABLE_PAT` resolves to a different underlying token depending on whether the deployment is Production or Preview, with the application code unaware of the difference. Verified working end to end afterward against real data — a real existing record correctly matched and rendered, and a genuine miss correctly created a new record — confirming the separation didn't just get configured but actually functions correctly.

**Abuse protection — built and verified live, not just flagged anymore.** A dedicated field, `Requester IP` (`fldpMXfKSokbxZrWU`), was added to `Compliance Lookup Events` specifically so rate-limit checks can use an exact match — a substring search against free text would have incorrectly matched IPs sharing a numeric prefix (e.g. `192.168.1.1` matching within `192.168.1.10`). The rate-limit check runs first, before any other Airtable operation for that request — a rate-limited request never reaches the search/create logic, so the thing actually being protected (Airtable writes, queue pollution) never happens regardless of what response gets returned. Threshold: 10 requests per 10 minutes per IP, deliberately generous so normal manual testing never trips it. Two limitations are documented in code comments rather than hidden: this isn't atomic (a genuine burst of near-simultaneous requests could each read "under threshold" before any finishes writing), and it's IP-based, so genuine users sharing a network share a limit too. **Confirmed live, precisely:** the limit fired on exactly the 11th real request after exactly 10 succeeded — not off by one in either direction — confirmed by reading the actual logged event count directly, not inferred from behavior alone. `Requester IP` was confirmed populated with a real address, not blank, on every logged event. All three original self-search cases were re-confirmed working unchanged after this was added, per the required verification order (Section 9.7).

**A platform-level decision made and worth recording precisely, since the reasoning matters as much as the setting:** Vercel's Firewall offers Bot Protection (Off/Log/Challenge) and AI Bots (Allow/Log/Deny) controls. Both are set to **Log**, not the stricter option, deliberately — not caution for its own sake. Sendr is expected to fetch this page's content programmatically for its video compositing (Section 2), which is exactly the kind of non-browser, possibly-AI-classified traffic these stricter settings exist to block. Turning either to Challenge or Deny before Sendr is connected and its real request pattern is verified risks silently blocking the one integration this project has been building toward, based on a guess about how it fetches content. Revisit both once Sendr is actually connected and its traffic is visible in these logs — at that point there's real evidence to act on, not a guess. Worth noting the AI Bots setting specifically has a real reason to eventually move to Deny — an AI crawler repeating a "clean" finding as current fact months later, without this project's own careful "as of the date checked" hedging, would undermine exactly what this project has been most careful about — but that's a decision for once Sendr's own traffic pattern is confirmed safe, not before.

**The two new UI states, with actual copy, confirmed live:**

*No match found* (a new record is created quietly in the background; not shown as such to the searcher) — reuses the existing gated `/r/:token` page unchanged, confirmed by screenshot: heading shows the real searched business name, body shows the existing gated copy. No separate "no match" copy exists in the build — an earlier draft of this spec proposed one; it was correctly simplified away before building, and the live result confirms that simplification works.

*Multiple matches* (a disambiguation list) — confirmed live via screenshot: "We found more than one match for '[search term]' in [jurisdiction]. Select your business:", each row showing `Trade/Operating Name`, both candidates correctly scoped to the selected jurisdiction.

A single exact match with `Consent Status` not yet consented reuses the existing gated placeholder from Section 8.3 unchanged — confirmed live on a real record, not just designed to.
...
**What "unmerged PR" actually means here, precisely:** the code is real, pushed, and verified against a live preview deployment with real Airtable reads and writes — not a local, untested draft. It is not yet on `main`, and has not been exercised against production traffic. Treat the functionality as proven; treat the deployment state as pending.

---

## 7. Quadrant Tag — A Real, Live Signal This Document Previously Had No Knowledge Of

`Quadrant Tag` (`fldBiFdD6z8E4e6qq`) is a live formula field on `Compliance Records`. **Corrected during self-search's live verification, not left as originally found** — see the fix below.

**A real bug found and fixed here, discovered by self-search, but not a self-search-specific problem.** The original formula's "pending" check only matched the literal select value `"Not Yet Checked"` — a genuinely blank field (the natural state of any record between creation and its first actual check, regardless of intake channel) fell through to the worst-case bucket instead. Confirmed directly: a real, brand-new self-search-created record with both status fields blank showed `Flagged-Flagged` — a "confirmed double-negative" reading for a record nobody had checked at all yet. This isn't a self-search-only issue; nothing suggests cold-outreach leads are any different before their Day-4 check runs, so this almost certainly applied to every record in the pipeline pre-check, not just this one path. Fixed and confirmed live — the exact same record now correctly shows `Pending / Insufficient Data` after the formula update, recomputed automatically with no manual intervention.

The corrected formula, live now:
...
This is a status-only 2×2 matrix — WSIB (clean/flagged) × Corporate (clean/flagged) — apparently meant to route Sendr's script to one of four variants. **This is a genuinely different concept from the display-tier logic the repository independently built** (a five-tier, date-driven "worst of the two" verdict system — see Section 9.3). Neither was built with knowledge the other existed: this document never mentioned `Quadrant Tag` until it was found, so the repo's tier system was built blind to it.

**Resolved, not open — decided explicitly, not inferred:** the front end never reads `Quadrant Tag` for anything. It stays entirely Sendr's concern, used only to pick a script variant. The front end keeps running its own status-and-date logic exactly as built (Section 8.2). The reasoning behind this: the two signals answer different questions for different audiences — `Quadrant Tag` answers "clean or flagged, right now" for automated script-selection; the front end answers "how urgent is this" for a human, which needs the date dimension `Quadrant Tag` was never built to carry. A certificate can be `Clean` on `Quadrant Tag` with three days of validity left — genuinely fine by one measure, genuinely urgent by the other. Neither system is wrong; they're not measuring the same thing, so there was never actually a conflict to reconcile.

**A related, adjacent gap flagged but deliberately not fixed here:** `Trigger Sendr Generation`'s own gate (Section 6.2, condition 4: `Corporate Status != "Not Yet Checked"`) has the identical blank-vs-literal-value gap — it only excludes the literal value, not a genuinely blank field. This is the automation's gating logic, a separate concern from this display-bucket formula, and needs its own decision rather than being folded into this fix.
...
### 8.2 Status color-coding — fully implemented and verified across all seven tiers, not still pending

**This was previously, incorrectly stated as not yet implemented in this document — that claim directly contradicted Section 9.4 a few paragraphs below it, and was simply stale.** The three-way scheme (green = genuinely good, orange = flagged and verified, gray = inconclusive or pending, no badge at all for blocked states) is built and confirmed live. This took three separate rounds of fixes, not one — worth knowing the sequence, since each looked complete on its own and wasn't:

1. The two originally-documented bugs (shared expiry threshold, status-override regex) — fixed as part of the real-data rewiring (Section 9.4).
2. Two more bugs found only by rendering a real record, not by reading the code — the footnote text being computed but never rendered, and the threshold legend firing for fields with no expiry date at all (Section 9.4).
3. **A third round, not previously documented here:** the `'lapsed'` tier itself was relabeled from `LAPSED` to `RECHECK NEEDED` and recolored from orange (matching `'flag'`) to the same gray used for `'watch'`/pending states — the original orange-and-"LAPSED" combination read as a confirmed negative finding when it was actually just an unverified, aged record. The overall verdict banner's `'lapsed'` entry (`VERDICT_COPY.lapsed`) got the same color and copy correction, since it had independently asserted "work sites and general contractors... will see a non-compliant status" — an unverified claim stated as fact. Separately, `VerdictBanner.tsx`'s spine accent bar had its own bug (a `backgroundColor: undefined` override silently clearing the color the `background` shorthand had just set) affecting all seven tiers, not just `'lapsed'` — found and fixed in the same round.

**Confirmed live, all seven tiers, spine color matched exactly against `VERDICT_COPY`:** `blocked`/`lapsed`/`pending`/`inconclusive` → `#4C5A67`; `flag` → `#C1501C`; `clear` → `#E1EEE6`; `watch` → the real striped gradient, not a flattened fallback. Nothing here is still pending.
...
**The data-population policy for when a new jurisdiction's fields don't line up with the existing schema — stated fully, not just in one direction:** the current field set is not to be expanded to accommodate jurisdiction differences. When a jurisdiction's source data doesn't provide an equivalent for a field that exists in the schema, that field is left blank on the record. When a jurisdiction's source provides something that has no corresponding field in the current schema, that data is not captured — it's discarded, not added as a new field. The schema stays exactly as it is; only which of its existing fields get populated varies by jurisdiction.
...
Two were the originally documented display-logic bugs; two more were found during live verification of the fix for those. **A further round — the `'lapsed'` tier's label, color, and banner copy, plus an unrelated spine-rendering bug affecting all seven tiers — is documented in Section 8.2, not repeated here, since it's a display-presentation fix rather than a classification-logic one.**
...
**The pattern worth carrying forward, not just this instance of it:** every one of these bugs, across both this section and Section 8.2's additional round, and three of the four deployment fixes in Section 5, looked correct on a code read and turned out to be wrong or incomplete only once tested against something real. Reading the code is necessary here; it has not once been sufficient on its own this session.
...
**A real, recurring obstacle found and permanently removed, not just worked around once:** Vercel's own preview-collaboration toolbar (Comments/Toolbar feature) repeatedly intercepted clicks and typed text meant for the actual app during live self-search testing, redirecting them into its own comment threads instead. This cost significant effort before being identified, since the failure looked like the app itself misbehaving. **Resolved at the root, not patched around:** the setting — labeled "Pre-Production Deployments" in Vercel's current UI, which appears to be the current name for what documentation elsewhere calls the Preview environment — was previously at "Default," meaning this project was silently inheriting whatever the Vercel team's own default policy was, not explicitly set on at the project level. It's now explicitly set to Off for this project specifically, in Project Settings → General → Vercel Toolbar, overriding the team default going forward regardless of what that default is or becomes. A platform-level setting, not a code change, with zero effect on Production, and fully reversible if ever wanted back. **One real scope limit worth being precise about:** this override is specific to this one Vercel project. It does not change the team-level default itself, so any separate, future Vercel project under the same team (a distinct front end for a future jurisdiction, for instance) would inherit that same default again and need this same override applied to it individually — this closes the loop for every future deployment of this project, not automatically for a different project that doesn't exist yet.

**A related, durable testing practice worth keeping even with the toolbar gone:** for verification that depends on an exact, repeated interaction count (like confirming a rate limit fires at a specific threshold), calling the live endpoint directly via JavaScript in the browser proved more reliable than repeated UI clicks — no DOM timing or page-load races to fight. Reserve actual UI-driven testing for what only it can confirm: what a real person actually sees and experiences. Use direct API calls for precise counting.
...
**A durable pre-merge check this session's credential-separation work exposed, worth applying before every future merge of a write-dependent feature, not just this one:** the Preview PAT (Section 6.3) was deliberately separated from Production's own token specifically so an unverified, in-progress feature couldn't touch Production's credential. That's correct while a feature is still being tested. But it creates a real, easy-to-miss trap at the moment that feature is actually ready to merge: Production's own token still carries only whatever scope it needed *before* this feature existed, not what the newly-merged code will actually require. Before merging any PR that writes to Airtable, confirm Production's own token has been separately given matching scope — don't assume Preview's dedicated testing token covers it, and don't assume "it was verified in Preview" means Production is ready to run it. These are two different credentials by design; verifying one says nothing about the other.
...
- **The `Jurisdiction` field was in the original spec from the start**, explicitly described as forward-compatible scaffolding — five options listed (`Ontario`, `British Columbia`, `Alberta`, `Quebec`, `US`), with an explicit build note to populate only `Ontario` until other markets actually launch. This is the source for Section 1.4's scope-boundary reasoning.
- **The four-system map in Section 2, and the Zapier leg specifically, both trace back to this same original spec, not to anything found by inspecting the live base.** The spec's `Trigger Sendr Generation` automation description includes a note that the return leg — Sendr writing its generated page URL back — was designed to go through Zapier to a GHL custom field, with pointing that same Zap at Airtable's `Sendr Page URL` field described as optional, for record-keeping only.
