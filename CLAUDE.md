...
**Abuse protection — still flagged, not built.** A public endpoint that creates records has no protection today against automated spam. Consistent with this project's pattern of observing real usage before building for a hypothetical, this stays a requirement to watch once real traffic exists — unchanged by the fact that the endpoint is now genuinely live in a preview environment reachable by anyone with the URL, even if unlisted.
...
- **Abuse protection on the self-search write endpoint** (Section 6.3) — flagged, not built. Watch once real traffic exists rather than engineer against a hypothetical now.
