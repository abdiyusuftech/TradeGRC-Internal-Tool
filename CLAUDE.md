...
- **A new, real open decision surfaced by this work, not resolved here:** whether `AIRTABLE_PAT` should stay scoped to Preview environments permanently now that it has write access, given that leaves every future PR's preview able to write real production data. Tracked in Section 12.
...
**The PAT write-scope requirement is resolved, not open.** `data.records:write` was added to the existing token, confirmed by Airtable's own documentation that this doesn't change the token's value — no Vercel redeploy was needed, and the live write tests confirm it took effect immediately. **One real, unresolved decision this surfaced, not a technical gap:** should `AIRTABLE_PAT` stay scoped to Preview environments permanently now that it has write access, or was that only meant for this round of testing? Leaving it on means *any* future PR's preview deployment can write real data to production Airtable, not just read it — a standing exposure, not a one-time setup step. Tracked in Section 12.
...
1. **Should `AIRTABLE_PAT` stay scoped to Preview environments permanently, now that it has write access** (Section 6.3), or should that be reverted to Production-only once self-search's PR merges? Leaving it on means every future PR's preview deployment can write real data to production Airtable, not just read it — a standing exposure, not a one-time testing convenience.
2. **Queue-priority ordering** (Section 6.3, Section 11) — should new leads or stale rechecks take priority when daily capacity is constrained, now that self-search adds a third competing source into the same queue. This is a business tradeoff between outreach throughput and data freshness, not a technical call.
