...
export const StatusBadges: React.FC<StatusBadgesProps> = ({ wsibField, corporateField }) => {
  return (
    <div id="status-badges-section" className="mt-4 sm:mt-5">
      ...
      {/* Threshold explanation, only when at least one field is on its expiry-tier scale — WSIB's
          ~42-day cycle and Corporate's ~5-year cycle use different bands (CLAUDE.md 9.4), so they're
          explained separately rather than under one shared line. */}
      {(wsibField.category === 'clear' || corporateField.category === 'clear') && (
